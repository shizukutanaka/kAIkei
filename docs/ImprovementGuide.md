...
### 改善7 🟡 源泉徴収の法定表（**残タスク**・金額の是正は完了）

**是正済み**: 月次の源泉所得税は、検証済みの法定計算（給与所得控除→社会保険料
控除・基礎控除・扶養控除→速算表→復興特別所得税）を年額で組み立てて12で割る
方式にした（`monthly_withholding.py`）。総支給の5%を掛ける実装は、月給40万で
約2倍の過大徴収、年収96万の非課税者からも徴収、高所得者からは徴収不足という
状態だった。扶養親族等の数（`employees.dependents` / migration 0031）も追加済み。

**社会保険料も是正済み**: `employees.birth_date` と会社ごとの料率（migration 0030）
を追加し、標準報酬月額の等級・折半端数・介護保険の要否まで法定どおり。賞与も
標準賞与額（1,000円未満切捨・年度573万/1回150万の上限）で計算している。

ここまでの教訓は **「概算なのは計算方法が無いからではなく、モデルが入力を
持っていなかったから」**。残りも同じ構造なので、まず入力から埋めること。

残作業:
- 「給与所得の源泉徴収税額表（月額表）」そのもの。所得税法189条の
  **電子計算機等を使用する特例**を使えば300行の表を持たずに計算できる。
  甲欄/乙欄の区分は `Employee` に未保持。
- 賞与の「算出率表」。前月の社会保険料等控除後給与と扶養親族等の数で率を引く。
  `bonus_withholding_tax.py` は率を受け取る形で実装済みなので、率の決定だけが残り。
- 別表の係数は**必ず国税庁の一次資料で確認する**こと。
